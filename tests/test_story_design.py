...
class StoryDesignManagerTest(unittest.TestCase):
    def setUp(self):
        ...
        specs=(("urs-maker","dummy","planner"),("planner","gemini","story-architect"),("concept-reviewer","dummy","planner"),("story-architect","gemini","continuity-reviewer"),("continuity-reviewer","dummy","story-architect"),("plotter","gemini","plot-reviewer"),("plot-reviewer","dummy","plotter"),("writer","gemini",None))
    ...
    def test_full_plot_flow_and_writer_boundary(self):
        ...
        self.assertTrue(Path(final["path"]).is_file()); self.assertEqual(final["next_agent"],"writer"); self.assertFalse(final["next_stage_implemented"])
    ...
    def test_mail_lineage_reaches_story_architect_plotter_and_writer(self):
        ...
        recipients=[r["recipient_id"] for r in rows]; self.assertIn("story-architect",recipients); self.assertIn("continuity-reviewer",recipients); self.assertIn("plotter",recipients); self.assertIn("plot-reviewer",recipients); self.assertEqual(recipients[-1],"writer"); self.assertIsNotNone(rows[-1]["parent_message_id"])
