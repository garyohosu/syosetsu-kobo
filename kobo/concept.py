...
class ConceptManager:
    ...
    def finalize(self,work_id=None,session_id=None):
        ...
        parent=session["source_mail_id"]; mail_id=self.orchestrator.mail.send("planner","writer",f"CONCEPT確定 work_id={session['work_id']} concept_path={path} 次工程=作品設計（未実装）",parent_message_id=parent) if parent else None
        with self.orchestrator.connection() as db:
            db.execute("INSERT INTO concept_documents(session_id,version,path,candidate_id,urs_path,created_at) VALUES(?,?,?,?,?,?)",(session["session_id"],version,str(path),candidate["candidate_id"],session["urs_path"],timestamp)); db.execute("UPDATE concept_sessions SET status='final',updated_at=? WHERE session_id=?",(timestamp,session["session_id"])); db.execute("UPDATE works SET current_agent='planner',next_agent='writer',status='pending',updated_at=? WHERE work_id=?",(timestamp,session["work_id"]))
        return {"path":str(path),"version":version,"candidate_id":candidate["candidate_id"],"urs_path":session["urs_path"],"mail_id":mail_id,"next_agent":"writer","next_stage_implemented":False}
